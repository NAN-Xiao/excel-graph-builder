...
class AbbreviationDiscovery(RelationDiscoveryStrategy):
    ...
    def _mine_abbreviations(self, graph: SchemaGraph):
        ...
        if self.patterns:
            self.logger.info(f"[Phase 2] 挖掘到 {len(self.patterns)} 个缩写模式: {list(self.patterns.keys())}")
    ...
    def _verify_match(self, from_table, from_col, to_table, to_col) -> bool:
        """验证两列内容是否匹配"""
        from_col_data = next((c for c in from_table.columns if c['name'] == from_col), None)
        to_col_data = next((c for c in to_table.columns if c['name'] == to_col), None)
